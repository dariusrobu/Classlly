    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    // 4. EXAMS SECTION (Updated to Full Width)
                    if !exams.isEmpty {
        ...
        .toolbar {
            ToolbarItem(placement: .primaryAction) {
                Button(role: .destructive) { showingDeleteAlert = true } label: { Image(systemName: "trash").foregroundColor(.red) }
            }
        }
        ...
        } message: { Text("Are you sure? All grades, attendance, and tasks for this subject will be deleted.") }
    }
}
...
// ✅ UPDATED: Full Width Exam Card
struct ExamCard: View {
    let exam: GradeEntry
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Image(systemName: "doc.text.fill").foregroundColor(.white)
                Spacer()
                Text("\(Int(exam.score))/10").fontWeight(.bold).foregroundColor(.white)
            }
...
// MARK: - UPDATED ROWS WITH COLOR

struct GradeRow: View {
    let grade: GradeEntry
    
    var gradeColor: Color {
        if grade.score >= 9.0 { return .green }
        else if grade.score >= 7.0 { return .blue }
        else if grade.score >= 5.0 { return .orange }
        else { return .red }
    }
...
struct AttendanceRow: View {
    ...
    var statusIcon: String {
        ...
        case .excused: return "person.crop.circle.badge.questionmark.fill"
        }
    }
    ...
    var body: some View {
        HStack {
            ...
            if let due = task.dueDate {
                Text(due.formatted(date: .abbreviated, time: .omitted)).font(.caption).foregroundColor(.red)
            }
        }
