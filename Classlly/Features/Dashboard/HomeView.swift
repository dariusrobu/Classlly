...
struct StandardDashboard: View {
    ...
    var body: some View {
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 16) {
                                    ...
                                    Button(action: { showAddSubject = true }) {
                                        ...
                                        .background(themeManager.selectedTheme.primaryColor.opacity(0.1))
                                        .cornerRadius(16)
                                    }
                                }
                                .padding(.horizontal)
        ...
        .sheet(isPresented: $showLogGradeSheet) {
            QuickLogGradeSheet(subjects: subjects)
        }
        .sheet(isPresented: $showGradeCalcPicker) {
                ...
                HStack {
                    ...
                    if let grade = subject.currentGrade {
                        Text(String(format: "%.1f", grade))
                            .font(.subheadline)
                            .fontWeight(.bold)
                            .foregroundColor(grade >= 5 ? .green : .red)
                    }
                }
...
struct QuickLogGradeSheet: View {
    ...
    @State private var weightValue: String = "100"
        ...
        NavigationStack {
            Form {
                ...
                if selectedSubject != nil {
                    Section("Grade") {
                        TextField("Value (1-10)", text: $gradeValue)
    ...
    var body: some View {
        NavigationStack {
            List(subjects) { sub in
                NavigationLink(destination: WhatIfGradeView(subject: sub)) {
                    Text(sub.title)
                }
            }
        ...
        NavigationStack {
            List(subjects) { sub in
                HStack {
                    Text(sub.title)
                    Spacer()
                    QuickAttendanceButton(subject: sub)
                }
            }
    ...
    var body: some View {
        Button(action: toggle) {
            Image(systemName: isAttended ? "checkmark.circle.fill" : "plus.circle")
                .font(.title2)
                .foregroundColor(isAttended ? .green : .blue)
        }
    }
...
// MARK: - OTHER GAME MODES (Stubs)
struct RainbowDashboard: View {
    let subjects: [Subject]
    let tasks: [StudyTask]
    var body: some View {
        Text("Rainbow Mode Active")
            .foregroundColor(.white)
            .padding()
