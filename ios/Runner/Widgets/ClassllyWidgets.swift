...
struct Provider: TimelineProvider {
    ...
    private func getData() -> UpNextEntry {
        ...
        let title = userDefaults?.string(forKey: "next_class_title") ?? "No classes"
        let time = userDefaults?.string(forKey: "next_class_time") ?? ""
        let location = userDefaults?.string(forKey: "next_class_location") ?? ""
        
        let taskTitle = userDefaults?.string(forKey: "next_task_title") ?? "No tasks"
...
// MARK: - Task Checklist Interactivity
...
struct ToggleTaskIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        // Logic to communicate back to Flutter or update local data
        // For now, we'll just log it. Real implementation would update group defaults.
        return .result()
    }
}
    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Spacer()
                Text(entry.nextTaskTitle).font(.headline).lineLimit(2)
                Text(entry.nextTaskDate).font(.caption).foregroundColor(.red)
                Spacer()
            }
            .padding()
        }
...
@main
struct ClassllyWidgetBundle: WidgetBundle {
